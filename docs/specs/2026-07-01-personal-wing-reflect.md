...
3. Behavior-smoke-test aimed at the discipline, not just parsing: appends rather than rewrites, skips promotion when nothing qualifies, refuses to manufacture a lesson, routes a surfaced decision onward. Ran as three parallel probe scenarios — 16/16 discipline checks passed; probe-surfaced text fixes folded (live-decision exclusion in the description, write-date semantics, append-only rationale, corpus slice-name check, distilled-seed referent).
4. Gitflow accommodation (JP-approved): the user-level `require-gitflow.py` hook gained additive `GITFLOW_ALLOW_FILES_EXTRA` / `GITFLOW_PROTECT_FILES_EXTRA` env vars that UNION with per-project overrides of the base lists; set user-level to allow `/Users/jp/personal/**` with `AGENTS.md`/`CLAUDE.md` kept protected. Verified by direct pipe-tests (union allow, protect veto, negative control) plus a real Write-tool proof on `main` in `~/personal`. The hook is capability tooling under the charter's scope rule — not a gated contract event, no ledger entry.
5. Output quality is proven only by JP's real use — first real reflection session seeds the journal; watch-and-prune from there.
