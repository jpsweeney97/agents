...
**Status:** v7 approval candidate — six review rounds folded; hand-authoring starts on JP's explicit approval · **Date:** 2026-07-13 · **Source:** this session's `outcome-shaping` → `making-recommendations` → `design-exploration` chain, with three external design reviews adjudicated via `review-reviewer` (all judged `reliable`; every confirmed correction incorporated), a fourth in-repo `scrutinize-skill` review folded as v5, and a fifth in-repo `scrutinize-skill` review adjudicated `reliable` via `review-reviewer` and folded as v6 — both confirmed P1 packet-contract defects plus the adjudication's two adjacent packet gaps, and a sixth design review adjudicated `reliable` via `review-reviewer` and folded as v7 — its three confirmed P1 contract gaps plus the three adjacent findings its adjudication added. Build-and-prune class (a user-invoked, read-only skill): **not a charter event**, no ledger entry owed.
...
2. **Prune** — receives the field with per-candidate provenance flags, its matrix share of the effective contract, and the prune method below — never the authority notes: Prune cuts lean-blind, and seed protection runs on provenance flags, which carry identity, not preference. Returns survivors plus labeled exclusion records, plus any disclosed budget overflow.
...
| `soft-prefs` — labeled soft preferences | — | ✓ | — | ✓ | ✓ |
| `evidence` — supplied inputs, known gaps, echoed authorization | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| `stakes` — reversibility, stakes, blast radius | — | — | ✓ | ✓ | — |
...
A pasted capsule's prior artifacts enter a packet only where this matrix names them (the re-run rule). Prune's lean-blindness is deliberate: cuts must be defensible without knowing the user's preference, and the excluded-favorite challenge belongs to Contest, which holds the excluded share.
...
- A change that only alters downstream admissibility (values, evidence scope, budget, degradation permission where execution-only) → re-prune the stored field, after the evidence-identity check clears it; degradation-permission-only changes preserve the field and re-echo execution mode.
