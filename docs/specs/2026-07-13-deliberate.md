...
**Verdict partition, reconciled.** `agent-facing-design`'s library convention reads: exploratory skills render no verdict, and a skill that both explores and adjudicates is two skills. `deliberate` composes rather than merges: exploration stays owned by `ideate`'s contract (no verdict rendered), adjudication by `making-recommendations`' (exactly one close), each executed in its own isolated context, and the run's only verdict is the one the decision-owning constituent renders under its own contract — never a bolted-on verdict over exploratory output, which is the failure the convention exists to prevent. As a skill, `deliberate` sits on the decision-owning side on purpose: trust machinery whose product ends in one close. If the convention's wording should name composed orchestrators explicitly, that is a one-line clarification to `agent-facing-design` proposed at hand-authoring, not assumed here.
...
| `retrievals` — earlier stages' returned web facts, each with producing stage, source, retrieval time | — | ✓ | ✓ | ✓ | ✓ |
...
**Read isolation is packet-field only.** Stages read the filesystem, so withheld-class content — raw decision wording, excluded identities, lean language — can exist inside authorized evidence, and the packet boundary does not protect against that. The contract says so instead of pretending: a stage that encounters withheld-class material inside evidence must not treat it as user authority and must report the encounter in its returned envelope's `encounters` field, and the proof boundary carries a read-isolation line (`packet-field isolation only; evidence-content encounters: none reported | <listed>`) parallel to the containment class.

## Stage output envelope

Every stage returns one transport envelope — transport, never a semantic form the judgment agent fills to feel done. Its fields:
...
- `retrievals` — every web fact the stage retrieved, each with source and retrieval time; `none` otherwise. Accepted retrievals join run state and travel onward only through the matrix's `retrievals` row.
...
Validation is mechanical and runs before acceptance: the envelope parses; every obliged artifact is present or honestly absent; every exclusion or disposition record is complete in the labeled shape, its `Option` matching the stored original wording — the orchestrator holds the field and survivors, so the comparison is exact, and a paraphrased record is rejected, never forwarded to Contest; and every record whose reason or epistemic status relies on an external fact carries its provenance. Anything else is `stage failed: <stage>` — the orchestrator never repairs a packet, invents a missing field, or accepts free-form output.
...
| Capability unavailable at preflight | that exit, pre-spend | echo only |
| Capability lost mid-run | `capability lost mid-run` — or inline continuation when degradation is permitted, labeled | echo, failure capsule (validated artifacts only) |
