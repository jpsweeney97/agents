...
- What is the smallest safe way to let runtime preflight use contract-derived values without importing first-party code before the census and without reading an unpinned policy file at invocation time? **Settled by the implementation plan (2026-07-16):** the entrypoint embeds the census-consumed policy subset as constants and compares that rendering against the contract's `import-boundary` section at every contract load, refusing on any difference. A pre-import `yaml.safe_load` read was rejected: it would give a pinned surface a first parse weaker than the validator's event-checked loader, and duplicating that loader pre-import would defeat the extraction. See the ADR-0001 amendment and `docs/plans/2026-07-16-deliberate-v6-shared-module-extraction.md`.
