...
**Trigger check (2026-07-16 scrutiny).** The prefixed-ZIP bypass reproduced during execution-readiness review is another ambient-import escape (a valid zip behind an inert-suffix prefix, loaded by zipimport), so it crosses this trigger's first clause and Option 2 must re-earn the decision. It does: the defect was in the census's *detector implementation* — a four-byte magic sniff instead of a structural check — not evidence that an in-place census fundamentally cannot see ambient artifacts. A structural detector (`zipfile.is_zipfile`) closes it within Option 2's own mechanism, keeps both consumers on one detector, and needs no launcher/staged-root. Option 2 is retained. Option 3 remains the escalation for a future bypass that structural in-place detection genuinely cannot close, or for the fragility-at-scale clause above.
...
For Option 2, the rollout should preserve the existing branch fixes as the tactical floor. First, land or rebase the current Gate-2 branch and refresh the live source identity. Next, choose an authenticated policy placement: inside the already pinned `contract-data.yaml`, in a new pinned method surface with the surface-set expansion accepted as part of v6, or in an unpinned editing reference whose generated values are embedded into an already pinned runtime surface and checked at release. The runtime must not consume an unpinned reference directly. Then add the declarative policy contract and a check that the current authoring checker consumes or renders the same values. Then implement the runtime pre-import preflight for the first v6 module extraction. The embedded policy values are authenticated two ways: they live in the entrypoint source, hashed as a method surface, and a non-executing release-time test asserts they equal the authenticated contract section — so a desynced pair cannot ship. That release-time proof is what satisfies "no unauthenticated code executes before method-surface identity is established"; a genuinely pre-import *runtime* equivalence check is architecturally impossible under this cut (the match needs the contract parsed, and the parser is the very first-party code being extracted — see the ADR-0001 addendum of 2026-07-16), so the per-invocation embedded-vs-contract compare runs post-import as defense-in-depth against a `--data` swap, not as the authentication gate. Finally, run the dual-path smoke through both canonical and Claude symlink paths, recording the effective interpreter.
...
- Add a policy-consumer mismatch test: remove one archive suffix, loader suffix family, allowed directory, or banned identifier from either consumer and observe a deterministic failure.
- Add runtime pre-import tests for the exact classes represented in `E005`: sourceless `.pyc`, `__pycache__`, symlinked module, symlinked package, package directory, `.zip`, `.egg`, `.whl`, disguised and prefixed zip (detected structurally), nested `.py`, shadowing local module, and benign inert files.
...
- Define the import-boundary policy contract with only values the checker and runtime truly need: module naming regex, allowed data directories, loader suffix families, archive suffixes (zip archives detected structurally, no magic list), banned identifiers, and cache-prefix invariants.
...
- Which authenticated placement should own the policy: the already pinned `contract-data.yaml`, a new pinned method surface with accepted surface-set expansion, or generated constants embedded into an existing pinned runtime surface from an unpinned editing reference? **Decided (JP, 2026-07-16):** the already pinned `contract-data.yaml` owns the policy, as a top-level `import-boundary` section; the other two placements are declined. Recorded in the ADR-0001 amendment of the same date.
- What is the smallest safe way to let runtime preflight use contract-derived values without importing first-party code before the census and without reading an unpinned policy file at invocation time? **Settled by the implementation plan (2026-07-16), refined by the execution-readiness scrutiny the same day:** the entrypoint embeds the census-consumed policy subset as constants, authenticated by the entrypoint's own method-surface hash and by a non-executing release-time test that asserts the embedding equals the contract's `import-boundary` section; `_require_boundary_match` re-checks that equality at every contract load as post-import defense-in-depth (a pre-import runtime match is circular — the parser is the extracted first-party code — so the release-time proof, not the per-invocation compare, is the authentication). A pre-import `yaml.safe_load` read was rejected: it would give a pinned surface a first parse weaker than the validator's event-checked loader, and duplicating that loader pre-import would defeat the extraction. The runtime census runs in two passes so zip detection can use `zipfile.is_zipfile` (structural, catching prefixed archives) only after shadows are cleared; there is no `zip-magics` value. See the ADR-0001 amendment and 2026-07-16 addendum, and `docs/plans/2026-07-16-deliberate-v6-shared-module-extraction.md`.
- Should Option 2 be part of the first v6 physical extraction, or should the branch land first and the policy-contract work become a separate pre-v6 gate? **Decided (2026-07-16, planning):** the branch landed first (`c6e5554..fc86e7b` on `main`), and the policy contract lands inside the single v6 cut as one `contract-data-version` bump to 6 — the validator's exact top-level key set means the section cannot land additively under version 5, and a second bump would double the capsule hard cut for no benefit.
- Is the accepted external `sys.path` residual still acceptable after the first module split, or should future ADR work add a static signal for source-tree `sys.path` mutation? **Open** — revisit after the first module split, per the rollout plan.
