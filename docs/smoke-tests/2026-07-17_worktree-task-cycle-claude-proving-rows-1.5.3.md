...
- **Status at this commit:** in flight — row 2 captured PASS and row 1's head verbatims recorded below; row 1's uninterrupted tail and row 4 run after this commit and their verbatims land in the closing cycle's commit (no commit claims its own future).
- **Date:** 2026-07-17
...
## Row 4 — plan (runs after this commit, recorded in the closing cycle's commit)

Row 4 (fabricated-foreign-lease adjudication on `work-router`): plant a fabricated foreign lease directly in the live lease root (fixture authority: design v3 §7's "previously scoped fabricated-lease manipulations remain authorized only for those proving fixtures, never for a real foreign lease", re-invoked by JP's 2026-07-17 instruction "Rerun Claude rows 1, 2, and 4" — the row is defined by that fixture); `lease-acquire` must refuse exit 2 printing the owner facts; `inspect` must map `LEASE-ORPHANED`; user-authorized break of the fabricated fixture (`trash`, agent-run, visible transcript); re-acquire and release must both exit 0 leaving the lease root empty.
