...
## Correction (2026-07-18, methodology-critique treatment)

The harness note above glosses the orchestrators' `claude-opus-4-8[1m]` self-reports as "the `model: sonnet` spawn override did not take on the background orchestrator." Transcript ground truth contradicts that gloss for run 1's orchestrator (`agent-aorchestrator-991e6c632976fe48`): its spawn request and `.meta.json` both record `model: sonnet`, and all 252 of its assistant turns carry `message.model: claude-sonnet-5` — the `[1m]` value came from a harness banner that misreported the model. Consequence: the run-1 failure capsule's `effective-models` entry `orchestrator: claude-opus-4-8[1m]` is a false value inside an accepted, validated capsule — the known specimen of the proof boundary's self-report gap, repaired at spec v29 by the effective-models provenance rule (`self-reported: <value>` / `unknown`, disagreements recorded as disagreements). This appendix corrects the gloss forward; everything above it is unchanged frozen history. Brief: `docs/reviews/2026-07-18-deliberate-methodology-critique.md`.
