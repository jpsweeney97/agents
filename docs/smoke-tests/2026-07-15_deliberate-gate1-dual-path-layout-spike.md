...
Correction 2 (2026-07-15, implementation review): Finding 5 and carry-forward input (1) phrase the mitigation as a "session-temp `sys.pycache_prefix`", which is insufficient as a design mandate. A follow-up isolated probe proved that reusing one prefix across invocations reproduces the stale-bytecode execution *inside* the prefix: after a same-size source edit with restored mtime, the second run executed the first run's cached code. Pass 2's evidence stands — its prefix was created fresh within the single spike — but the binding v6 requirement is a freshly created, initially empty, invocation-private prefix that is never reused, with retirement owned by the entrypoint; recorded in ADR-0001's cache-neutralization consequence together with its four verification obligations.
