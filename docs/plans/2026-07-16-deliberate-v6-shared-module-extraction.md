...
## Current state (verified 2026-07-16 at `672abb5`)

- `main` holds the landed Gate-2 arc; this branch adds the decision folds, this plan, and one Gate-2 repair (`672abb5`, below). Worktree clean; full suite 39 passed in ~62s; Gate-2 checker exit 0 with "agree: 1 Python surface(s)"; fixtures 158/158.
...
### Prerequisite landed on this branch (`672abb5`) — prefixed-ZIP Gate-2 repair
...
| `skills/deliberate/tests/check_import_closure.py` | modify | census/ban values loaded from the target root's `contract-data.yaml` `import-boundary` section (structural zip detection via `zipfile.is_zipfile` already landed in `672abb5`) |
| `skills/deliberate/tests/test_import_closure.py` | modify | `make_layout` writes the policy section; live-tree test expects 2 surfaces; policy-floor, policy-binding, and embedded-vs-contract-equality tests (prefixed-ZIP regression already landed in `672abb5`) |
...
| `docs/smoke-tests/fixtures/2026-07-14-deliberate-exact-prompt.txt` | exists (`e071b3f`) | byte-exact 3,760-byte `$deliberate` smoke prompt, SHA-256 `253f1bfe…`; the durable, hash-pinned Task 7 input (README beside it records provenance) |
...
- `_is_zip_archive(path)` stays policy-free — it calls `zipfile.is_zipfile` (structural detection, already landed in `672abb5`); no magic list to thread.
