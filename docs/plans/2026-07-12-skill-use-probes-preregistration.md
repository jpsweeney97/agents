...
- Post-verdict addendum (2026-07-12, same bounded no-gate-effect framing as the offer note above): the raw table also carries a take-signal counterpart — mid's ON arm took `simplify-code` for its main task in 2 of 3 trials while OFF took no skill at all (0 of 3). Outside the sealed key (which counted only `characterization-tests` takes), n=3, spotted post-verdict: hypothesis-generating only, changing no verdict.
