...
revised: 2026-07-21
sealed: 2026-07-22
status: "SEALED 2026-07-22 — this revision is the seal; the commit landing it is the seal event. The sealed run executes only on JP's separate, explicit authorization (sequencing step 5). Sealed-map SHA-256: b200a4095ff21d26da6a494d524d3884f689a455e58058b4d0aa8bf6c52aa216 || RUN CLOSED 2026-07-25 — authorized 2026-07-22, executed across four sessions, stopped on the 15-operator-hour ceiling at 322/400 dispatches. Verdict INCONCLUSIVE (W2): positive control undetermined and adjudication unrun. Human arm closed unadministered; results composed into the Results section. Amendments 1-7 ratified during execution. Prior status text preserved verbatim above per the no-rewrite discipline. || SUCCESSOR 2026-09-03 — a known-answer run on deliberate 2.0's Prune (docs/plans/2026-09-03-t2-known-answer-check/) put ten authored cases through Prune once each: seven hidden winners kept, three cut, every cut record's Revive if naming the fixing fact. A conditional-form answer for version 2.0, not a rate on real decisions. Prior status text preserved verbatim above per the no-rewrite discipline. || CONTEST TEST 2026-09-03 — case 06's cut winner was carried through Shape, Recommend, and Contest the same day (docs/plans/2026-09-03-t2-known-answer-check/README.md, section The Contest test on case-06): Recommend re-added it under making-recommendations' add-an-option rule on the full evidence and rendered a clear call for it; Contest did not name the cut. On that one case the wrong cut did not reach the user, and the recovery came from Recommend's contract, not from the cut ledger or the exclusion check. Two method repairs followed: decide 2.2.0 (Contest names a cut whose substance the close adopted) and decide 2.2.1 (Prune's fact-established label keyed on the record's own Revive if; issue #21). One case, one dispatch per stage. Prior status text preserved verbatim above per the no-rewrite discipline."
...
## Amendment 2 — dispatch-ceiling raise to 400 [ratified by JP 2026-07-24]

Forward correction, disclosed per the dated-appendix discipline; nothing above this section is rewritten. Amendment 1 stands unchanged except its clause-4 sentence "The 300/15 ceilings are unchanged and remain hard stops," which this amendment supersedes as to the dispatch number only.

**Event record.** At the ARM P → ARM C boundary (189 of 300 dispatches committed, ~6 of 15 operator-hours), the remaining collection was priced against the remaining ceiling and does not fit. The ARM C rep wave is 98 dispatches (7 cases × k=7 reps × the Appendix C two-pass transport); dual-blind extraction is 14 (7 cases × 2 extractors); that is 112 against 111 remaining, before any kill, and the run's observed kill-and-replace rate adds roughly 8 more. The adjudication layer — sealed at neutral re-shape, blind packet rewrite, executed leak probe, three same-family and two cross-family judges per divergence, plus the decoy floor of at least two decoys per real packet — is unbudgeted entirely. Under the unamended ceiling the run would stop mid-extraction at roughly 287 committed, compute no gate, and report INCONCLUSIVE with its ARM C data collected but never read.

**Why the decision was forced to this moment.** Adjudication cost scales with the divergence count, which is outcome-bearing. A later, better-informed ceiling request would be sized by that count and would therefore disclose it in a channel JP observes, before his cold-judge adjudications are recorded — the precise failure the sequestration rule exists to prevent. The ceiling was therefore re-decided outcome-blind, at the last moment at which no adjudication data exists.

**Ruling (JP, 2026-07-24, AskUserQuestion on the session transcript, ratified as stated).**

1. The dispatch ceiling is raised from 300 to **400**. The operator-hours ceiling is **unchanged at 15**. Both remain hard stops; hitting either still stops data collection, and the run still reports at whatever gate its collected data supports, never backfilled.
2. **This is the final ceiling amendment.** No further raise will be requested or granted once any extraction or adjudication data exists, because the size of such a request would leak the divergence count. If 400 dispatches or 15 operator-hours are exhausted, the sealed Cost clause governs without further appeal.
3. The raise is an instrumentation change only. It touches no inference parameter: k = 7, the capped N, the FALSIFIED threshold of 2, the ARM C health floor, the readable-run floor, every blinding rule, and every gate definition are untouched, so it cannot move the verdict in either direction.
4. Budget allocation on the record at ratification: ~120 dispatches to reach the gate computation (98 rep units + 14 extraction units + kill headroom), ~90 held as the adjudication reserve. The allocation is a projection, not a sub-ceiling; the 400 total is the only binding number.
5. The final report discloses this amendment, the arithmetic that forced it, and the fact that the run's original ceiling was mis-sized — the mini-pilot priced ~216 dispatches for nine cases but could not price the adjudication layer, the recovery-driven redos, or the three prune honest exits that spent two reserve cases.

## Amendment 3 — close-scan seam vocabulary declared [ratified by JP 2026-07-24]

Forward correction, disclosed per the dated-appendix discipline; nothing above this section is rewritten. This amendment fills an open bracket the seal left, and declares no new parameter beyond it.

**Event record.** The prereg carries two distinct seam instruments. The ARM C **merge** seam-word scan is explicitly flag-and-confirm with an operator allowlist route, and its eighteen-word list is carried in `merge_scan.py`. Separately, one sentence in Arms pre-registers a **close** scan with a different remedy: "a pre-registered seam-leak scan flags any ARM C rep whose close cites pass/batch structure; that rep is discarded and re-run." No word list was ever declared for the close scan — not in Arms, not in "Parameters fixed at the seal," not in appendices A–C. At the first ARM C rep wave the operator provisionally reused the merge list, which is calibrated for shaper output where any cross-candidate reference is itself a leak, and is therefore over-broad for a recommender close that legitimately compares the whole field.

**Evidence, gathered outcome-blind before referral.** No extraction had run and no winner-set existed for any rep of the affected case. Every seam flag in the wave was the single word "subset"; structure-vocabulary hits across all seven returned closes were zero; "subset" occurred zero times in the case's own rendered brief; the case's merge-stage allowlist was empty, so the merge had introduced no batch structure for a close to cite; and the seven already-accepted closes of the first completed case score zero under both candidate lists, so no available option disturbed accepted data.

**Ruling (JP, 2026-07-24, AskUserQuestion on the session transcript, ratified as stated).**

1. The close-scan **discard set** is structure-only — vocabulary that can only describe the shaping apparatus: `batch`, `tranche`, `merged`, `assembl*`, `pass one`, `pass two`, `first pass`, `second pass`, `earlier section`, `above section`, `previously shaped`. A hit on any of these discards the rep and re-runs it, with no confirmation route.
2. The remaining merge-list words — `subset`, `cohort`, `grouping`, `merge`, `other candidates`, `remaining candidates`, `the other options` — are **domain vocabulary for close-scan purposes**. They are recorded descriptively per rep and never discard a rep. They remain governed by the unchanged flag-and-confirm rule at the merge stage, where the operator allowlist route continues to apply.
