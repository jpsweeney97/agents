...
2. The app must keep sending the engagement analytics events the store's featuring team audits. What it costs: removing analytics from the app is out.
3. No new paid services or vendor contracts. What it costs: paid crash-analysis upgrades, new SDK vendors, and contractors are out.
...
Grouped by what each option changes: which build users run, the third-party analytics library, the app's own code, and the release or the users. Order within a group is not a quality order.
...
## Change the analytics library

**Update the analytics SDK to its latest release** — Bump the third-party analytics library to the newest version the vendor has published, and ship.
Sets it apart: a dependency bump; it changes nothing the team wrote.

**Remove the analytics SDK** — Take the third-party analytics library out of the app entirely.
Sets it apart: removes one whole dependency; the app then reports nothing about engagement.

**Replace the analytics SDK with another vendor's** — Swap the current analytics library for a different provider's.
Sets it apart: keeps analytics while abandoning the current library; a migration rather than a fix.

**Move analytics to a server-side pipeline** — Send raw events to the team's own backend and compute engagement there, removing the client library's logic from the app.
Sets it apart: puts the team in control of the whole analytics path; the largest engineering move in the field.
