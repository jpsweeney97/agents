...
- `.agents/scratch/skill-use-probes/` — the probe rig (git-ignored via the `.agents/` ignore rule; verified in `.gitignore`). Subdirs: `trees/` (two scratch `CLAUDE_CONFIG_DIR` trees + pristine tarballs), `fixture/` (frozen workspace fixture + manifest), `armmap/` (arm map + sealed keys + hash record + seal marker + the executor-maintained environment record + the extend-pinned first-round grades snapshot; never readable by the scorer), `tasks/` (fresh-authored trial prompts), `out/` (captured trial streams + `results-index.json` + stripped scorer packets); plus `runner.py`, `synthetic-suite.py`, `schedule.json`, `pilot-schedule.json`, `schedule-extensions.json` (runner-derived rows only, written by `runner.py extend` and equivalence-checked on every load — the sealed `schedule.json` is never edited and the extension file is never hand-edited), and the exact-roster check inputs `expected-roster.json` and `sentinel-absent.json` at the rig root, joined at run time by `grades.json` (the scorer's merged offer grades) and `pilot-notes.md` (Task 6.4).
...
# derivation exactly, with the first-round grades digest pinned at extend
# time and every stream's capture digest re-checked at packet time. The
# scorer NEVER runs this and never reads armmap/.
...
TRIAL_TIMEOUT = 1800  # seconds per trial; a timed-out trial is invalid and re-runs as a replacement row
...
def hash_inventory() -> dict:
    ...
    schedule-extensions.json, armmap/grades-snapshot.json, and the out/
    streams are deliberately absent: the first two legitimately change
    post-seal (checked by derivation equivalence and the sealed-load digest
    comparison), and each stream is pinned per-row at capture instead (its
    SHA-256 lands in the results row; packets re-checks it)."""
...
def runtime_identity() -> dict:
    ...
    return {"claude_version": v.stdout.strip(), "model": MODEL,
            "permission_mode": PERMISSION_MODE, "trial_timeout": TRIAL_TIMEOUT,
            "env": ENV}
...
def first_round_digest(rows: list, grades: dict) -> str:
    """Digest of the standing first-round grade values (escalation rows
    excluded), pinned by `extend` whenever escalation rows are derived and
    re-checked on every sealed load: first-round grades stand (sealed rule),
    so a grade edit that leaves the derived row set unchanged still surfaces
    — as this mismatch instead of a changed result."""
    ids = sorted(r["trial_id"] for r in rows if not r.get("escalation"))
    sub = {t: grades[t] for t in ids if t in grades}
...
def derive_extensions() -> list:
    ...
    a hand edit to the extension file surfaces as an equivalence mismatch,
    and a grade edit surfaces here only when it changes this derived row
    set; edits that leave it unchanged are caught by the extend-pinned
    first-round grades digest (first_round_digest), not by equivalence."""
...
def load_schedule(pilot: bool = False) -> list:
    ...
    if pilot:
        ...
        if any(r.get("escalation") for r in required):
            snap = load_json(RIG / "armmap" / "grades-snapshot.json")
            live = first_round_digest(base + required,
                                      load_grades(RIG / "grades.json"))
            if live != snap.get("first_round_grades_sha256"):
                sys.exit("schedule failed: first-round grades changed after the "
                         "escalation derivation was pinned — first-round grades "
                         "stand (sealed rule); investigate the edit before any "
                         "re-pin. Got: digest mismatch")
        sched = base + required
        if (bad := sorted(t["trial_id"] for t in sched
...
def run_trial(trial: dict) -> dict:
    ...
    with out_path.open("w") as out:
        try:
            ...
            return {"trial": trial, "valid": False, "stream_sha256": sha256(out_path),
                    "why": f"trial timed out after {TRIAL_TIMEOUT}s — invalid, re-run"}
    if r.returncode != 0:
        return {"trial": trial, "valid": False, "stream_sha256": sha256(out_path),
    ...
    return {"trial": trial, "valid": valid, "takes": takes,
            "stream_sha256": sha256(out_path), "why": why}
...
def packets() -> None:
    ...
    results = load_json(RIG / "out" / "results-index.json")
    ...
    for r in results:
        ...
        if by_id.get(tid) != r["trial"]:  # same row-identity guard as tally
            sys.exit("packets blocked: results-index row does not match its "
                     f"scheduled row: {tid}")
        sp = RIG / "out" / f"{tid}.jsonl"
        if not sp.exists():
            sys.exit(f"packets blocked: results row without its stream: {tid}")
        if sha256(sp) != r.get("stream_sha256"):
            sys.exit("packets blocked: stream does not match its capture "
                     f"digest: {tid}")
        events = [json.loads(l) for l in sp.read_text().splitlines() if l.strip()]
...
if __name__ == "__main__":
    ...
    if cmd == "record":
        ...
        if any(r.get("escalation") for r in required):
            snap = first_round_digest(load_json(RIG / "schedule.json") + required,
                                      load_grades(RIG / "grades.json"))
            (RIG / "armmap" / "grades-snapshot.json").write_text(
                json.dumps({"first_round_grades_sha256": snap}, indent=2))
            print(f"pinned first-round grades digest: {snap}")
    elif cmd == "run":
...
check("G3 packets refuses a results row not matching its scheduled row",
      ...
      p.stdout + p.stderr)
d = rig([r1_], keys=POS_TAKE, ext=[])
stream = '{"type":"result","result":"done"}\n'
(d / "out" / "p-A-r1.jsonl").write_text(stream)
ok_row = res(r1_, takes=[])
ok_row["stream_sha256"] = hashlib.sha256(stream.encode()).hexdigest()
(d / "out" / "results-index.json").write_text(json.dumps([ok_row]))
p = run(d, "packets")
check("G4 packets passes a stream matching its capture digest",
      p.returncode == 0 and json.loads(
          (d / "out" / "packets" / "p-A-r1.json").read_text())["final_text"] == "done",
      p.stdout + p.stderr)
(d / "out" / "p-A-r1.jsonl").write_text(stream + '{"type":"result","result":"edited"}\n')
p = run(d, "packets")
check("G5 packets blocks a stream edited after capture",
      p.returncode != 0 and "capture digest" in p.stderr, p.stdout + p.stderr)

# ---- Group H: escalation grades-snapshot pin
s = sched33()
g, rx = grades_for(s, {"mid-A-r1", "mid-A-r2"})  # ON 2-1 -> EXTEND fires
d = rig(s, keys=KEY33, grades=g, results=rx)
run(d, "extend")
g_edit = dict(g)
g_edit["mid-B-r1"] = {"offers": ["ideate"]}  # in-window edit: derived row set unchanged
(d / "grades.json").write_text(json.dumps(g_edit))
p = run(d, "tally")
check("H1 in-window first-round grade edit blocks via the pinned digest",
      p.returncode != 0 and "first-round grades changed" in p.stderr,
      p.stdout + p.stderr)
d = rig(s, keys=KEY33, grades=g, results=rx)
run(d, "extend")
(d / "armmap" / "grades-snapshot.json").rename(d / "armmap" / "grades-snapshot.missing")
p = run(d, "tally")
check("H2 escalation rows without the pinned snapshot block",
      p.returncode != 0 and "grades-snapshot.json" in p.stderr,
      p.stdout + p.stderr)
