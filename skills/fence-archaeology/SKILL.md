...
description: "Use when about to remove, disable, or weaken something that looks useless or obsolete — code, config, a guard clause, a process step, a checklist gate — and the reason it exists is not already known and current; also when rationalizing a question set or process. Digs the reason out of the record and renders a per-fence disposition (reason-dead / reason-alive / reason-unknown) before any removal. Do not use when the reason is documented and current, to map consumers of an interface change (contract-change-propagation, where available), or to resolve which source of truth governs a claim (baseline, where available)."
---
...
5. **Render the disposition, one row per fence:**
   - **Reason found, dead** → remove; put the evidence the reason expired into the removing commit's message, so the next archaeologist inherits it. For a fence with potential live consumers — a system, module, endpoint, or flag — a dead reason alone is not enough: measure whether anything still uses it from a runtime signal (request metrics, access logs, telemetry, dependency-analysis output) rather than concluding absence from a clean static search, because a static-clean surface can still be depended on (Hyrum's Law). Where no runtime signal is reachable, say so and treat the surface as unverified-live, not dead — dispose of it as reason-unknown.
