---
name: deploy-plan
description: "Use when a specific risky change is about to ship and needs a rollout decision before push — rollout shape, an explicit go/no-go, pre-registered abort thresholds and a bake window — plus the post-push read of those thresholds to call the rollout healthy or aborting. Advisory only: it never executes a deploy, traffic shift, or rollback. Not for deriving a version or changelog (`release-cut`), authoring a reusable operational runbook (`runbook-authoring`), or checking whether the goal later moved (`outcome-check`)."
---
...
- **Advisory-not-actor** — recommend; the operator executes. On `abort`, point at the characterized rollback path; if prod is actually harmed, stabilizing it is live incident work outside this skill.
