...
2. Roots/instructions/status: run status first. From the target directory, or the parent directory for a file target, run `git rev-parse --show-toplevel`; if it fails, use the explicit target root as a non-git root and state that git diff/status evidence is unavailable. Before planning edits, discover and read applicable instructions: repo-root `AGENTS.md` or `CLAUDE.md`, any nearer instruction file between the root and target path, and local package/tool config that controls formatter, linter, tests, or generated files. Honor an in-source do-not-simplify marker where the repo uses one: a block whose comment records why it must stay as written (a measured optimization, a bug workaround, a spec-mandated shape) is out of scope for the pass — name it in the Review Packet's exclusions rather than silently leaving it alone — and treat a comment stating a non-obvious reason for the current shape as a fence for `fence-archaeology`, not a candidate for removal.
...
6. Broad scopes: do read-only reconnaissance. List candidate slices with files, call sites, simplification, risk, planned verification strength, lane eligibility, and remaining candidates. When reconnaissance shows the sites are one uniform mechanical edit, follow SKILL.md `## Use`'s instrument-switch routing to `migration-campaign` instead of slicing further. Patch one coherent slice. Value order: defect risk, cognitive load, meaningful duplication, local patterns, test/debug ease, then aesthetic ride-alongs. Closeout must state when the original broad request was not exhausted.
...
Exit `0` means every scoped editable file has a restorable copied pre-edit file. Exit `1` means an artifact was created but one or more scoped files were excluded and are not restorable from that artifact; get explicit approval for the named restore limit, choose a secure backup mode, pick a safer target, or stop. Exit `2` means no valid backup artifact was produced; stop before editing. The helper also exits `2` when `--planned-verification` does not begin with `strong plan`, `moderate plan`, or `weak plan`.

The helper's `--allow-exclusions` flag makes a run with excluded files return exit `0` instead of exit `1`; it does not make the excluded files restorable. Pass it only after the user has explicitly approved each named restore limit, and never to silence the exclusion check.
...
Nothing enforces the recorded retention: the artifact persists until manually removed with `trash`, and cleanup after the simplification is accepted is the user's or a directed session's work, not this skill's. Record the retention expectation honestly and do not imply automatic cleanup.
...
10. Failure/closeout: if verification fails, do root-cause analysis first. Use baseline to separate pre-existing from new failure; fix only with clear cause/path. Follow repo-local commit instructions. If they require a commit, stage only scoped files after successful verification. If no commit is requested or repo-required, leave changes unstaged unless asked, but closeout and review prompts must make that state intentional instead of self-blocking: say the scoped changes are intentionally unstaged because no commit package was prepared, and phrase staging as a future pre-commit step, not a current blocker. Do not ask reviewers to block on unstaged files unless commit packaging is in scope for that review.

When commit packaging is in scope, prepare the package before review instead of outsourcing obvious staging defects to the reviewer. Stage the coherent scoped file set together after successful verification, including new destination files and source deletions for moves or splits, then run `git diff --cached --name-status` and `git diff --cached --check`. If the cached package is incomplete or dirty state makes safe staging ambiguous, fix it before closeout or report that as the blocker. A review packet that asks reviewers to verify staging must include the cached file list or say explicitly that packaging was not prepared.
