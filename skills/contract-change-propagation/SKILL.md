...
- the **lock, online-index, backfill, and drop-ordering safety of running a schema or DDL change against a *live* database** → `migration-safety`. The two compose along different axes on a DDL change: this skill owns the compat axis (which consumers break, what semver, rollout order), `migration-safety` owns the DB-execution axis — do the consumer/compat mapping here and hand the live-execution-safety plan to it.
