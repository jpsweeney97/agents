...
- **Pointer repair precedes inlining.** A must-have target behind a weakly worded pointer is a variance bug: sharpen the pointer's wording first, and inline the material only when sharpening fails. A skill description and an `AGENTS.md` line naming a doc are the same object under the same rules.
- **Invocation permission is a deliberate axis, paired across runtimes.** Skills split on who can fire them. Model-invoked is the default — model or user can reach it, and the description keeps rich trigger phrasing so auto-invocation fires. User-invoked means only the human typing its token: `disable-model-invocation: true` in the Claude frontmatter, paired with `policy.allow_implicit_invocation: false` in `agents/openai.yaml` where that companion exists — a Claude-only gate leaves the skill model-reachable on Codex, so keep the pair in sync: user-invoked in both runtimes or neither. A user-invoked description is a human-facing summary for the skill picker, not model-routing text. The deciding test: could the model usefully reach for this autonomously? Two consequences follow. No skill can reach a user-invoked skill — a step whose precondition is one is phrased as an instruction for the human to run it (`/x` or `$x`), never as an operative invocation. And gating is a design choice to make on purpose: gate a skill whose mis-fire or stolen fire is costlier than the routing it loses, not by default.
