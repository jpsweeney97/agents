...
- **Output probes for the target's own vocabulary.** Upgrading "in context" to "produced this kind of output" by grepping assistant output for a name the body itself carries (an exit route in a routing table, an instrument's label) counts echoes of the loaded body and passing mentions, not the behavior. Real behavior often does not wear the body's names: a route to evidence reads "run a small test", not the table's skill name. A zero from such a probe is not deadness and a hit is not an exit. Report the row as a mention count, and when a finding turns on whether the behavior happened, tag the rider and escalate rather than resting it on the count.
