...
6. Rank every serious option, recommend one only if evidence supports it, and label readiness.
...
- Low stakes: use a concise shape, usually `Recommendation`, `Why`, `Trade-off`, and `Readiness`. Include gaps only when they matter.
- Medium stakes, high stakes, or when the user asks for depth: use the fuller packet below.
