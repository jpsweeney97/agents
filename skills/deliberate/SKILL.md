...
- **`option-shaping` (Shape):** the provenance seam ships as a source edit in `option-shaping` itself (composition-workflow candidates accepted on evidenced invocation provenance); the evidence reaching Shape is the `composition-provenance` packet item — the elided verbatim invocation and delegation spans from setup decomposition, because authorization is evidenced, never asserted; the constraint-consequence split and collision terminals are run-context packet rules layered on top.
...
One principle: **restart at the earliest stage whose input or required artifact became invalid** — matrix-derived, with named exceptions and the fixed directive-normalization order in `references/capsule.md`. At re-run setup, store creation is `import-capsule`: the helper validates the pasted capsule and writes the typed restart state, and the renderer reads a prior stage's artifacts from the imported capsule exactly until a re-run envelope supersedes them — no prior envelope is ever synthesized. Evidence authority is live, checked by content identity; constituent and method drift converts to invalidation via the source-drift map; revival is a full active-state transition followed by the standing cardinality branches (the mechanical lookup rides the import as `--revive`); a pasted capsule's prior judgments enter a stage packet only where that stage's checklist names them.
...
uv run --script $V import-capsule --data $D --store <root>/deliberate-run-live --run <id> --capsule <capsule.yaml> \
  [--revive <wording>] [--constraint-withdrawn <wording>] [--accept-seed <wording>] [--echo-body <echo.yaml>]
```
