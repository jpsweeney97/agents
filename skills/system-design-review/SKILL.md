...
If no material findings surface, say so directly. End with 2-4 sharp questions, not a verdict, unless requested. Save to `docs/audits/YYYY-MM-DD-<target-slug>.md` only when asked or when that workspace already uses `docs/audits/`.
