...
| --- | --- |
| dependency | deps, locks, imports, licenses, workspaces; record version skew, unused deps, upgrade drag, lockfile drift, license, compatibility, and maintenance cost only. If CVEs, GHSAs, exploitability, package-audit output, or vulnerability claims surface, stop that branch and route it to `codex-security:security-scan`; do not use the security signal as debt evidence |
