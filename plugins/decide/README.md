...
| `decision-record` | `docs/adr/NNNN-slug.md` at the next number, created lazily; on a supersession, a `Status` line on the older record; on a narrowing that leaves the older record in force, a cross-reference in the new record's prose plus, where the repo's ADR convention appends dated amendment sections, one such section below the older record's text; then a local commit of only those files (`docs(adr): record NNNN <slug>`), skipped on a protected branch or amid unrelated dirty state. Never pushes, opens a PR, or publishes. |
