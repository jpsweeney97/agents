...
description: "Use when an idea, plan, design, or decision is still muddy: a shaping interview to work out what the user wants before the next move. Not for one-off clarification, reviews, audits, or implementation."
---

<!-- export: plugins/decide/skills/outcome-shaping/ @ 946da914955aedbccd71398e2260e5775ebce5ad | 2026-09-23 | claude.ai -->
...
- When you are working from a summary rather than the user's actual sentences — after the conversation has been condensed, or deep in a long conversation — say so, and re-confirm the settled shape in their words before any capsule. Anything you cannot source to their words is your compression, and the capsule marks it that way.
- Keep the lane read-only, and hand off by name when the work shifts (see Exits).
- This method needs a live, responsive human correcting the read as it goes. When no one is there to correct it — the request asks for a finished capsule with no back-and-forth, or the user has said they will not be answering questions — say what the shaping would have surfaced and stop: report the underspecification rather than interviewing an absent user and treating your own read as their want.
...
- **Missing information** — the answer lives in reality, not introspection: would it be fast enough, do users care, does the approach even work. Name the evidence that would settle it and route toward it — a throwaway prototype built to answer just that question, an experiment, a measurement — instead of asking questions the user can only answer by guessing.
...
When the user points at an artifact, plan, or prior decision — pasted into the conversation or uploaded as a file — read what makes the next read materially better: before the first question when asking blind would be performative, or mid-interview when an answer reveals that context would clarify more than another question would. When they point at something you cannot see, ask for the piece that would help rather than guessing at it, and never imply you read what was not shared. Follow relevance, not a file cap, and stop once you can offer a better read or a better question.
...
| The answer lives in reality, not introspection                       | a throwaway prototype built to answer just that question, or gather the evidence                         |
| The blocking unknown is knowledge only one named person can close    | conversational closure: the capsule names that person and the work that remains, a question set for them (no handoff) |
...
Each destination named in backticks is a separate skill that may or may not be available where you are running. When it is available, name the handoff normally. When it is not, say so, return the short capsule with the kind of work that remains, and stop; do not perform the missing skill's work in this lane.
...
Omit any beat with no real content rather than manufacture one. The capsule carries no authority the shaping never earned, and briefs are chat-only: write a file, ticket, or durable artifact only when the user explicitly asks. When they do, the capsule is the brief: hand it over as a downloadable file or a copyable block with a plain filename, and leave placing it to the user — nothing you write persists beyond this conversation unless they save it.
...
- "Read this first" — inspect the named artifact (pasted or uploaded) before the first question.
