...
description: "Use when creating or materially changing agent-facing prompts, skills, rules, tools, schemas, or validators: a gate before adding structure agents must obey. Not for plain product code or user docs."
---
<!-- export: skills/agent-facing-design/ @ 48e319f0fe8054d67c0154ad71bb50df70625fac | 2026-09-23 | claude.ai -->

# Agent-Facing Design

Keep agent-facing systems useful in the moment. This skill leads two lives: as a design gate it is not a ritual but a pause before adding structure that future agents must obey; as a skill library's quality constitution it is the canonical home of the two-kinds-of-skill lens and the offload doctrine that sibling skills read and defer to.
...
If the target already exists, inspect the live target and nearby authority before deciding: work from what the user uploaded or pasted, and ask for the controlling surface when it was not shared. Read only enough to name the current obligation, the work product at stake, and the surface that controls it: prompt text, skill body, metadata, referenced examples, schema, validator, hook, script, repo instruction, or workflow doc. If a needed surface is unavailable, say what is unverified.
...
The per-edit grain hides the one thing a single verdict cannot see: accumulation. Run finding by finding through a review loop and each increment can be locally justified while the surface ratchets past the work it protects — a run of consecutive "justified" verdicts on one growing surface is itself the borderline case. Before the next yes, step back from the increment to the whole surface; if it now feels larger than the work it protects, simplify before adding more. This is the whole-surface question — run it here, per edit, not only when a reference happens to be opened. When that look suggests the original value-for-cost bargain may itself have changed — the work has changed category, or repairs now mainly protect machinery earlier repairs added — the continued-investment decision belongs to a separate, deliberately requested pass that reconstructs the original value-for-cost bargain and puts continue-or-stop to the human, not to a unilateral simplify-or-continue call here; this gate keeps per-edit and design-size judgment either way.
...
These conventions run through the skill library this gate was written for, as practiced but previously unwritten law; they are stated here so a new or edited skill lands on the right side deliberately rather than by drift. All are context for the designing agent, not a validator.

- **Verdict partition.** Exploratory and adversarial skills (`ideate`, `steelman`, an imagine-it-already-failed pass, an attacker-modeling exercise) render no verdict: their output is deliberately one-sided or divergent, and a bolted-on verdict gets mistaken for the calibrated call, short-circuiting the owner that should weigh the full picture. Decision-owning skills (a done-or-not closeout check, a ship-or-abort rollout call, a goal-met-or-not outcome read) render exactly one. Place a new skill on one side on purpose; a skill that both explores and adjudicates is two skills. A composed orchestrator is not that merged skill: when each stage executes a single constituent contract in an isolated context and the run's only verdict is rendered by the decision-owning constituent under its own contract, the orchestrator sits on the decision-owning side.
- **Close packet.** A skill whose output gates a downstream action or chains into another skill closes with a compact labelled field:value shape (a closeout check's verdict block, a rollout plan's bake-read) so the next reader — human or skill — can branch without re-parsing prose. Read-only, exploratory, and dialogic skills keep their native close; forcing a packet on them is waste, not discipline.
...
- **Branching disclosure test.** The decision rule for what belongs in a `SKILL.md` body versus `references/`: inline what every branch of an invocation needs; push behind a pointer what only some branches reach, with the pointer naming when to load it.
- **Pointer repair precedes inlining.** A must-have target behind a weakly worded pointer is a variance bug: sharpen the pointer's wording first, and inline the material only when sharpening fails. A skill description and a repo-instruction line (an `AGENTS.md` line, say) naming a doc are the same object under the same rules.
- **Invocation permission is a deliberate axis, paired across runtimes.** Skills split on who can fire them. Model-invoked is the default — model or user can reach it, and the description keeps rich trigger phrasing so auto-invocation fires. User-invoked means only the human typing its token, set through whatever the runtime offers for it; when a skill ships to more than one runtime, set it in every runtime or none — a gate set in only one leaves the skill model-reachable in the other. A user-invoked description is a human-facing summary for the skill picker, not model-routing text. The deciding test: could the model usefully reach for this autonomously? Two consequences follow. No skill can reach a user-invoked skill — a step whose precondition is one is phrased as an instruction for the human to type its token, never as an operative invocation. And gating is a design choice to make on purpose: gate a skill whose mis-fire or stolen fire is costlier than the routing it loses, not by default.

One surface convention joins them: maintainer-facing lifecycle and merit material — prune conditions and watch-signals, merit arguments, per-part judgment/trust declarations, provenance history — does not belong in a `SKILL.md` body. The body is the fire-time surface an agent reads mid-invocation, in whatever repo the skill fires in, so give it only what changes behavior at fire time; lifecycle and merit context lives in maintainer-facing lifecycle notes or the repo docs instead.
...
- for skill construction, hand-author against this gate plus a deliberate pass on the skill's user experience (how it is invoked, steered, and recovered from, and how its output lands), or use a constructor workflow where the target runtime bundles one
- for quantitative skill benchmarking or trigger optimization, use a measured with-and-without evaluation run, not this gate's judgment
- use `scrutinize` for critique or review of an artifact, and `scrutinize-skill` when the target is a skill
...
For implementation work, apply the smaller clearer design directly only when the owning edit path is already clear — here that means an edited copy of the uploaded file or revised text the user applies, never a change to their live source. After direct edits, validate through that owning workflow and state the proof boundary: structural checks on the copy prove parsing and shape, not that a realistic invocation followed the behavior, and the user's live source, plugin caches, marketplace metadata, distributed copies, and other runtime surfaces need their own checks in the user's environment when that surface is part of the claim.
