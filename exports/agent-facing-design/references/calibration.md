...
- "I can draft that evidence line instead, but I will not substitute it for the requested field unless you want the smaller design."
...
- An error that names what to try next is context; one that only reports what broke leaves the agent guessing. A house error format like `{operation} failed: {reason}. Got: {input!r:.100}` is one such shape, not a required one.
...
Cap orchestration depth at one hop and keep the merge in the calling agent: a delegate that itself delegates must summarize to hand off, so each added layer costs context fidelity and tokens while adding no decision. When a fan-out's results need combining, the caller combines them; it does not spawn a combiner. The same rule already stands in three concrete forms: a review panel spawns no nested panels; delegates return findings and the caller writes the report; and a design pass reads the load-bearing files itself and never designs from summaries alone (`design-exploration` states that for its own lane).
