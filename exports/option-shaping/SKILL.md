---
name: option-shaping
description: "Use when 2+ named options are sketch-level: develop to comparable depth, rank-free. Not to widen the field (ideate), clarify a muddy outcome (outcome-shaping), or choose (making-recommendations)."
---

<!-- export: plugins/decide/skills/option-shaping/ @ 6d0d9c594cef973af37d37e5e506b0ceea5aad88 | 2026-09-23 | claude.ai -->

# Option Shaping

Turn a fixed field of sketch-level options — fixed by the user (see Freeze the Field) — into a rank-free comparison surface. Equalize resolution — how much is actually understood about each option — not certainty, word count, or favorability.

## Freeze the Field

- Work on exactly the candidates the user selected. No orchestrating skill runs this copy, so a brief that claims its candidates were fixed by one is treated as the user's own selection: the supplied field stays frozen either way, and the ask-the-user behavior below is unchanged. Do not choose a promising subset, add alternatives, generate replacements, or silently drop an awkward option.
...
If the outcome is still too muddy to derive live questions (`outcome-shaping`), the field needs widening (`ideate`), the user now wants a choice (`making-recommendations`), or one approach needs to become an approved design (`design-exploration`), name that neighboring skill and ask before switching. Do not silently continue into it.
...
Prefer the smallest surface that makes the live distinctions inspectable. Use adaptive prose or a compact side-by-side; when several questions are in play, organize around the questions or use a table rather than emitting one polished card per option. Preserve the supplied field order — saying that it is the order the user supplied and not a ranking, so a later reader does not take a lean from it — unless a clearly non-evaluative organization improves readability.
