---
name: steelman
description: "Use when the user wants the strongest one-sided case FOR a position — 'make the best case for X', 'argue the other side'. Committed brief, honest close, no winner. Not for ranking or flaw-hunting."
---

<!-- export: skills/steelman/ @ 991bae20e31d1f6c47b708c3e4b4bd222e1e6bc5 | 2026-09-23 | claude.ai -->

# Steelman

Build the genuine strongest case *for* one position — usually one the user is inclined to dismiss — then stop without picking.
...
A reasoned brief in the response — the pinned position, the affirmative case, the visible load-bearing structure, the honest close. No ranking, no winner, no even-handed comparison. Nothing is persisted: a one-sided brief saved as a file is a confirmation-bias hazard in reverse — found later without its close, it reads as a balanced verdict it never was. If the user wants it kept, that is a separate write-up step in which the close travels with the brief; do not grow a persistence part here.
...
- The threat model is a motivated adversary, or the question is how a settled plan fails by accident, imagined from the future — that is attacker modeling or failure-imagination, not advocacy.
