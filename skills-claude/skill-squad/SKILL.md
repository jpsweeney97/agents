...
- **Assert the plumbing on the script's first lines.** Throw immediately unless every large constant is defined and non-empty and every field read from `args` is present, so a data bug fails on line one instead of mid-fan-out.
- **Keep large text in the one robust channel: backtick template literals inlined in the script body.** Prefer inlining over `args` for any substantial content — `args` is an easy place to silently lose a large object — and never assemble large prose from single-quoted strings, where an apostrophe ends the string early. This is the same instinct as the prose-not-schema rule under *What This Skill Will Not Do*: keep judgment-sized text out of every fragile channel.
