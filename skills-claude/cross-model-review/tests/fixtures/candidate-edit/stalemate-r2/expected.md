...
- **Outcome**: the first sentence says that no certificate was earned, which of section 3's cases closed the run, and that no sealed probe accepted a final text; the second says what that means for the user.
- **Where it stands** (in place of **Answer**): the last live proposal, or both positions when the models disagreed, in readable numbered structure, labelled as not accepted by both sides.
- **Decision needed** (in place of **Asserts / does not assert**): the decision the note hands to the user, in the user's terms.
- **Follow-ups owed** and **Pointers** as today; Pointers name `run.json` and, for a probe-refusal close, the evidence-dir record.

The chat presentation follows the file: outcome first ("no certificate; the run was closed because ..."), the `answer.md` path, and the decision under a "Need from you:" line. No section implies that an answer was accepted.
...
- **Close-out note**: the host-written prose the close carries verbatim: the reason, the pre-registered bounds and per-arm reasoning where probes refused, the transport-failure history, and the decision owed. For a probe-refusal close it is ADR-0030's timestamped close-out record, written into the evidence dir before the final attempt and carried by the op afterwards. Avoid: certificate, classification, verdict, run ending record.
...
- `~/.claude/skills/synapsis/SKILL.md` lines 8, 22, 34, 72, 78, 85-95, and 101: the three-way ending becomes certificate or close; "honest typed stalemate" is removed; the arm-playbook list at line 34 drops `scope-ambiguity.md` and `cap-exhausted.md`; the `answer.md` template at lines 85-95 gains the close form of section 4.6 beside the certificate form; the presentation rule for a close: no certificate, the decision handed back under "Need from you". This file is unversioned; the implementer records its pre-edit sha256 in the landing record.
