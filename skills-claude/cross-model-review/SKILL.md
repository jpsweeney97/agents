...
When a closing check has been recorded, `finish` renders an ending only after re-validating the checked candidate's saved reviewer record by replay. It refuses when that record is missing, invalid, or disagrees with the accepted response, and no other outcome can then be rendered, because `failed` requires a failed or pending call. Confirm that `finish` succeeded before presenting or linking its output. If it refuses, report its diagnostic and the evidence that remains (candidate snapshots, request and response records) directly to the user, state what cannot be verified, and neither reconstruct a record nor present an earlier result as the current ending.
