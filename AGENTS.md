...
- `plugins/<name>/` holds the canonical dual-runtime plugin sources (currently `handoff`, `review-family`, `git-cycle`, `relay`, `plan-cycle`, `decide`) in Claude format: `.claude-plugin/plugin.json` plus `skills/`. One source serves both runtimes; never create per-runtime copies.
...
- The GitHub release mirror is a separate repo (`jpsweeney97/codex-tool-dev`), not a path under this one: its live checkout is `/Users/jp/Projects/active/codex-tool-dev` and the mirror tree is that checkout's `plugins/turbo-mode/`. Identify the mirror by this path, not by remote: any clone of the repo shares the same `origin`, so the remote cannot distinguish the mirror from a copy. It is updated only at explicit publish time by copying from this repo's sources.
